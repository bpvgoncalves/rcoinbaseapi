...
#'
#'
#' List Coinbase Brokerage Accounts: Retrieves a list of brokerage accounts associated with the
...
#'
#' @return A data frame containing account details. The result includes all pages if pagination is required.
#'
#' @details
#' This function wraps the `/api/v3/brokerage/accounts` Coinbase API endpoint.
...
account_list <- function(limit = NULL, cursor = NULL, use_sandbox = FALSE) {
  ...
  ret <- resp_data$accounts

  while (resp_data$has_next) {

    resp <- apirequest("GET",
                       "api/v3/brokerage/accounts",
                       query_params = list(cursor = resp_data$cursor),
                       need_auth = !use_sandbox,
                       use_sandbox = use_sandbox)
